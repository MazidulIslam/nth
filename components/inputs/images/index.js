  ...
  const handleImages = (e) => {
    ...
    files.forEach((img, i) => {
      if (images.length == 1) {
        dispatch(
          showDialog({
            header: "Maximu 1 images are allowed.",
            msgs: [
              {
    ...
    <div className={styles.images}>
      ...
      <div className={styles.images__main}>
        <div
          ...
          // ${images.length == 2? styles.grid__two: images.length == 3
          //     ? styles.grid__three: images.length == 4 ? styles.grid__foor : images.length == 5 ? styles.grid__five
      ...
      {/* <button
        ...
        style={{ opacity: `${images.length === 1 && "0.5"}` }}
      ...
      <button
        type="reset"
        disabled={images.length === 1}
        style={{ opacity: images.length === 1 ? "0.5" : "1" }} // Opacity changes based on length
        onClick={() => fileInput.current.click()}
        className={`${styles.btn} ${styles.btn__primary}`}
      >
        {text}
